...
const TaskModal = ({ isOpen, setIsOpen, id }) => {
    ...
    useEffect(() => {
        if (isOpen && id?.projectId && id?.id) {
            console.log("Fetching Task with:", id);  // ✅ Debugging
    
            axios.get(`http://localhost:8080/api/project/${id.projectId}/task/${id.id}`)
                .then((response) => {
                    setTaskData(response.data);
                    console.log("Task Data:", response.data);
                })
                .catch((error) => {
                    console.error("API Error:", error.response);  // ✅ Logs full error details
                    toast.error('Task not found. Please check the project and task IDs.');
                });
        }
    }, [isOpen, id]);
                            ...
                            <Dialog.Panel className="rounded-md bg-white max-w-[85%] w-[85%] h-[85%] overflow-y-hidden">
                                ...
                                <div className='flex gap-4 h-[inherit]'>
                                    <div className="!w-8/12 px-8 space-y-3 py-4 min-h-max  overflow-y-auto">
                                        ...
                                        <p className='text-gray-600'>{capitalizeFirstLetter(taskData.topic)}</p>
