...
const AddTaskModal = ({ isAddTaskModalOpen, setAddTaskModal, projectId = null, taskId = null, edit = false, refreshData }) => {
    ...
    useEffect(() => {
        if (edit && isAddTaskModalOpen) {
            axios.get(`http://localhost:8080/api/project/${projectId}/task/${taskId}`)
                .then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!edit) {
            axios.post(`http://localhost:8080/api/project/${projectId}/task`, { title, topic, description: desc })
                .then((res) => {
            ...
            axios.put(`http://localhost:8080/api/project/${projectId}/task/${taskId}`, { title, topic: topic, description: desc })
                .then((res) => {
                    setAddTaskModal(false)
                            ...
                            <Dialog.Panel className="rounded-md bg-white w-6/12">
                                ...
                                <form onSubmit={handleSubmit} className='gap-4 px-8 py-4'>
                                    ...
                                    <div className='mb-3'>
                                        ...
                                        <input value={topic} onChange={(e) => setTopic(e.target.value)} type="text" className='border border-gray-300 rounded-md w-full text-sm py-2 px-2.5 focus:border-indigo-500 focus:outline-offset-1 focus:outline-indigo-400' placeholder='Task title' />
                                    </div>
                                    <div className='mb-2'>
                                    ...
                                    <div className='flex justify-end items-center space-x-2'>
                                        ...
                                        {!edit ? <BtnPrimary>Save</BtnPrimary> : <BtnPrimary>Update</BtnPrimary>}
                                    </div>
                                </form>
